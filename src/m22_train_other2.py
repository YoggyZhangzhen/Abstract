...
def train(dataset, val_dataset, v, start_epoch=0):
    ...
    if d10_config.scheduled_sampling:
    ...
    for epoch in range(start_epoch, d10_config.epochs): # 外层for循环 控制epoch多少轮
        ...
        # for batch, data in enumerate(tqdm(train_dataloader)): # 内层for循环 控制多少批次数
        for batch, data in enumerate(train_dataloader):  # 内层for循环 控制多少批次数
            ...
            # if (batch % 100) == 0:
            if (batch % 100) == 0:
                print(f'Average loss for epoch {epoch}, batch{batch}', np.mean(batch_losses))
        ...
        print('epoch:{}'.format(epoch), 'training loss:{}'.format(epoch_loss),
