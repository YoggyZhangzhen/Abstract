...
class Predict():
    ...
    '''
    # 问题1：如果搜素宽度=3，第1个时间步预测了几次，第2个时间步预测了几次，第3个时间步预测了几次，第n个时间步预测了几次？
            第1个时间步预测了1次，    挑选预测结果的前3名（top=3）
            
            第2个时间步预测了3次，    每一次挑选预测结果的前3名（top=3），一共生成9种结果
                                   通过小顶堆从9种结果中，选择得分最好的前三名
                                
            第3个时间步预测了3次，    每一次挑选预测结果的前3名（top=3），一共生成9种结果
                                   通过小顶堆从9种结果中，选择得分最好的前三名
            
    # 问题2：每个时间步的运算结果是什么？是如何存储的？
            每个时间步的运算是 (x, log_probs[x], decoder_states, coverage_vector)
            是如何存储的：
                生成一个新的beam容器，并且把第1步解码结果、第2步的解码结果、第3步的解码结果，给串联，存储起来
            # best_k = [beam.extend(x, log_probs[x], decoder_states, coverage_vector) for x in topk_idx.tolist()]
                    
    # 问题3：第t个时间步是如何拿到初始化数据的？也即是是如何拿到t-1步的运算结果的？
            答： # 当前候选集是堆元素的index=2的值can.
